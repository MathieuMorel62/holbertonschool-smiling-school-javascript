$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function(xml) {
            $(xml).find("quote").each(function (index) {
                ...
                appendQuoteElement(picUrl, name, title, text, index);
            });
    ...
    $.ajax({
        ...
        success: function(xml) {
            $(xml).find("video").each(function (index) {
                ...
                const star = parseInt($(this).find('star').text());
                const title = $(this).find('title').text();
                const subTitle = $(this).find('sub_title').text();
                const duration = $(this).find('duration').text();
                appendCardElement(
                    author, authorPicUrl, thumbUrl, star, title, subTitle, duration, index, contentSelector
                );
            });
            ...
            while ($(carouselSelector + " .carousel-item").length < 5) {
                $(xml).find("video").each(function () {
                    ...
                    const duration = $(this).find('duration').text();
                    appendCardElement(
                        author, authorPicUrl, thumbUrl, star, title, subTitle, duration, -1, contentSelector
                    );
                });
            }
  ...
  function populateSearchFilters() {
    $.ajax({
        ...
        dataType: "xml",
        success: function(xml) {
            let search = $(".section-filters input").val($(xml).find("q").text()).val();
            ...
            $(xml).find("topics topic").each(function () {
                ...
                $("#topic .dropdown-menu").append(
                    `<a class="dropdown-item f-medium px-4 py-2" data-name="${topic}" href="#">${formatText(topic)}</a>`
                );
            });
            ...
            $(xml).find("sorts sort").each(function () {
                ...
                $("#sortBy .dropdown-menu").append(
                    `<a class="dropdown-item f-medium px-4 py-2" data-name="${sort}" href="#">${formatText(sort)}</a>`
                );
            });
  ...
  function displayCourses(search = "", topic = "", sort = "") {
    ...
    let url = `https://smileschool-api.hbtn.info/xml/courses?q=${search}&topic=${topic}&sort=${sort}`;
        ...
        success: function(xml) {
            $(xml).find("course").each(function () {
                const course = {
                    ...
                    sub_title: $(this).find('sub_title').text(),
            ...
            let courseCount = $(xml).find("course").length;
